package org.runebase.wallet.utils.migration_manager;
...
import static org.runebase.wallet.utils.crypto.KeyStoreHelper.RUNEBASE_PIN_ALIAS;
...
public class MigrationManager {
    ...
    private static KeystoreMigrationResult migratePassphrase(Context context) {
        if (TextUtils.isEmpty(RunebaseSharedPreference.getInstance().getKeystoreMigrationState(context))) {
            String encryptedSaltPassphrase = RunebaseSharedPreference.getInstance().getSeed(context);
            if (!TextUtils.isEmpty(encryptedSaltPassphrase)) {
                try {
                    KeyStoreHelper.createKeys(context, RUNEBASE_PIN_ALIAS);
                    byte[] bytes = KeyStoreHelper.decryptToBytes(RUNEBASE_PIN_ALIAS, encryptedSaltPassphrase);
                    savePassphraseSalt(context, bytes);
                    RunebaseSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.SUCCESS.name());
                    return KeystoreMigrationResult.SUCCESS;
                } catch (NoSuchProviderException | NoSuchAlgorithmException | InvalidAlgorithmParameterException | RuntimeException e) {
                    return KeystoreMigrationResult.ERROR;
                }
            } else {
                RunebaseSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.NOT_NEED.name());
                return KeystoreMigrationResult.NOT_NEED;
            }
        } else {
            return KeystoreMigrationResult.NOT_NEED;
    ...
    private static void migratePinCode(Context context) {
        boolean isSixDigit;
        String encryptedPinHash = RunebaseSharedPreference.getInstance().getSixDigitPassword(context);
        isSixDigit = !TextUtils.isEmpty(encryptedPinHash);
        if (!isSixDigit) {
            encryptedPinHash = RunebaseSharedPreference.getInstance().getPassword(context);
            if (TextUtils.isEmpty(encryptedPinHash)) {
                return;
            }
        }
        String decrypt = KeyStoreHelper.decrypt(RUNEBASE_PIN_ALIAS, encryptedPinHash);
        if (isSixDigit) {
            RunebaseSharedPreference.getInstance().saveSixDigitPassword(context, decrypt);
        } else {
            RunebaseSharedPreference.getInstance().savePassword(context, decrypt);
        }
    }
    ...
    private static void savePassphraseSalt(Context context, byte[] saltPassphrase) {
        ...
        RunebaseSharedPreference.getInstance().saveSeed(context, base64);
    }

    private void clearWallet(Context context, Realm realm) {
        RunebaseSharedPreference.getInstance().forceClear(context);
