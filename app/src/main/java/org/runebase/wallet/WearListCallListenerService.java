...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = RunebaseSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            if (RunebaseApplication.instance == null || RunebaseApplication.instance.getWearableMessagingProvider() == null) {
                upSocketService();
            } else {
                sendData();
            }
    ...
    private void sendData() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                histories = RunebaseApplication.instance.getWearableMessagingProvider().getOperations();
                String balance = RunebaseApplication.instance.getWearableMessagingProvider().getBalance();
                String uncBalance = RunebaseApplication.instance.getWearableMessagingProvider().getUnconfirmedBalance();
                String address = RunebaseApplication.instance.getWearableMessagingProvider().getAddress();
                Gson gson = new Gson();
